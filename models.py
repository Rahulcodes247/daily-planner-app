...
class User(Base):
    ...
    provider = Column(Enum(AuthProvider), default=AuthProvider.GOOGLE, nullable=True)
...
class Reflection(Base):
    ...
    output = Column(JSON, nullable=True)  # List of dicts: [{'activity': ..., 'percentage': ...}]
    created_at = Column(DateTime, server_default=func.now())
...
class Plan(Base):
    ...
    optional_activities = Column(JSON, nullable=True)  # List of dicts: [{'activity': ..., 'hours': ...}]

    status = Column(Enum(PlanStatusEnum), default=PlanStatusEnum.start)
    output = Column(JSON, nullable=True)  # List of dicts: [{'activity': ..., 'start_time': ..., 'end_time': ...}]
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, server_default=func.now())
